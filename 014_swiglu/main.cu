...
__global__ void swiglu_kernel(const float* input, float* output, int halfN) {
    ...
    if (idx < halfN) {
        ...
        output[idx] = x * sigmoid * input[halfN + idx];
    }
}
...
int main() {
    ...
    int N = 4;
    int halfN = N / 2;
    float input[N] = {1.0, 2.0, 3.0, 4.0};
    float expected[halfN] = {2.1931758, 7.0463767};
    float output[halfN];
    ...
    swiglu_kernel<<<blocksPerGrid, threadsPerBlock>>>(input_d, output_d, halfN);

    CUDA_OK(cudaPeekAtLastError());
    CUDA_OK(cudaDeviceSynchronize());
    CUDA_OK(cudaMemcpy(output, output_d, halfN * sizeof(float), cudaMemcpyDeviceToHost));
