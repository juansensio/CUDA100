...
def torch_relu_kernel(input):
    return torch.nn.functional.relu(input)  # PyTorch implementation

input = torch.randn(3, device='cuda', dtype=torch.float32)
c1 = torch_relu_kernel(input)
c2 = module.relu_kernel(input)
assert torch.allclose(c1, c2, atol=1e-5)

Ns = [1000, 100000, 1000000, 100000000] 
rounds = 10
...
for size in Ns:
    ...
    num_bytes = size * 4 * 2  # float32, read+write, bytes moved
    ...
    print(f"{size:<10} {custom_mean_ms:<18.4f} {custom_gbps:<18.2f} {pytorch_mean_ms:<18.4f} {pytorch_gbps:<18.2f} {speedup:<10.2f}")
